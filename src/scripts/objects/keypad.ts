...
import { enablePlayerControl } from "./player";
...
export function setDoor(mesh: THREE.Mesh) {
  door = mesh;
  door.getWorldPosition(doorWorldPos);
  doorWorldPos.add(DOOR_PIVOT);
}

const DOOR_PIVOT = new THREE.Vector3(-1, 0, 0);

export function updateKeypad(delta: number) {
  ...
  if (doorOpen && door.userData.body) {
    point.position.copy(doorWorldPos);
    rotatePhysicsObject(door.userData.body, doorWorldPos, delta * 4);
  }
}
